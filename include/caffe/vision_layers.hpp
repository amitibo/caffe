 ...
 * @brief Convolves the input image with a bank of learned filters,
 *        and (optionally) adds biases.
 *   This is engine with simple reference implementation for convolutional layer
 *
 */
template <typename Dtype>
class SimpleConvolutionLayer : public BaseConvolutionLayer<Dtype> {
 public:
  /**
   * @param param provides ConvolutionParameter convolution_param,
   *    with ConvolutionLayer options:
   ...
   *    kernels + stream parallelism) engines.
   */
  explicit SimpleConvolutionLayer(const LayerParameter& param)
      : BaseConvolutionLayer<Dtype>(param) {}

  virtual inline const char* type() const { return "Simple Convolution"; }
