...
class SimpleConvolutionLayerTest : public MultiDeviceTest<TypeParam> {
 ...
 protected:
  ...
  virtual ~SimpleConvolutionLayerTest() {
    delete blob_bottom_;
...
TYPED_TEST_CASE(SimpleConvolutionLayerTest, TestDtypesAndDevices);

TYPED_TEST(SimpleConvolutionLayerTest, TestSetup) {
  typedef typename TypeParam::Dtype Dtype;
  ...
  shared_ptr<Layer<Dtype> > layer(
      new SimpleConvolutionLayer<Dtype>(layer_param));
  ...
  layer.reset(new SimpleConvolutionLayer<Dtype>(layer_param));
  layer->SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
  EXPECT_EQ(this->blob_top_->num(), 2);
...
TYPED_TEST(SimpleConvolutionLayerTest, TestSimpleConvolution) {
  typedef typename TypeParam::Dtype Dtype;
  this->blob_bottom_vec_.push_back(this->blob_bottom_2_);
  this->blob_top_vec_.push_back(this->blob_top_2_);
  LayerParameter layer_param;
...
TYPED_TEST(SimpleConvolutionLayerTest, Test1x1Convolution) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  ConvolutionParameter* convolution_param =
      layer_param.mutable_convolution_param();
...
TYPED_TEST(SimpleConvolutionLayerTest, TestSimpleConvolutionGroup) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  ConvolutionParameter* convolution_param =
      layer_param.mutable_convolution_param();
...
TYPED_TEST(SimpleConvolutionLayerTest, TestSobelConvolution) {
...
TYPED_TEST(SimpleConvolutionLayerTest, TestGradient) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  ConvolutionParameter* convolution_param =
      layer_param.mutable_convolution_param();
  ...
  SimpleConvolutionLayer<Dtype> layer(layer_param);
  GradientChecker<Dtype> checker(1e-2, 1e-3);
  checker.CheckGradientExhaustive(&layer, this->blob_bottom_vec_,
      this->blob_top_vec_);
}

TYPED_TEST(SimpleConvolutionLayerTest, Test1x1Gradient) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  ConvolutionParameter* convolution_param =
      layer_param.mutable_convolution_param();
...
TYPED_TEST(SimpleConvolutionLayerTest, TestGradientGroup) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  ConvolutionParameter* convolution_param =
      layer_param.mutable_convolution_param();
...
template <typename Dtype>
class CuDNNSimpleConvolutionLayerTest : public ::testing::Test {
 protected:
  CuDNNSimpleConvolutionLayerTest()
      : blob_bottom_(new Blob<Dtype>(2, 3, 6, 4)),
        blob_bottom_2_(new Blob<Dtype>(2, 3, 6, 4)),
        blob_top_(new Blob<Dtype>()),
        blob_top_2_(new Blob<Dtype>()) {}
  ...
  virtual ~CuDNNSimpleConvolutionLayerTest() {
    delete blob_bottom_;
    delete blob_bottom_2_;
    delete blob_top_;
    delete blob_top_2_;
...
TYPED_TEST_CASE(CuDNNSimpleConvolutionLayerTest, TestDtypes);

TYPED_TEST(CuDNNSimpleConvolutionLayerTest, TestSetupCuDNN) {
  ...
  shared_ptr<Layer<TypeParam> > layer(
      new CuDNNSimpleConvolutionLayer<TypeParam>(layer_param));
  layer->SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
  EXPECT_EQ(this->blob_top_->num(), 2);
  EXPECT_EQ(this->blob_top_->channels(), 4);
  EXPECT_EQ(this->blob_top_->height(), 2);
  ...
  convolution_param->set_num_output(3);
  convolution_param->set_group(3);
  layer.reset(new CuDNNSimpleConvolutionLayer<TypeParam>(layer_param));
  layer->SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
  EXPECT_EQ(this->blob_top_->num(), 2);
  EXPECT_EQ(this->blob_top_->channels(), 3);
  EXPECT_EQ(this->blob_top_->height(), 2);
...
TYPED_TEST(CuDNNSimpleConvolutionLayerTest, TestSimpleConvolutionCuDNN) {
  Caffe::set_mode(Caffe::GPU);
  this->blob_bottom_vec_.push_back(this->blob_bottom_2_);
  this->blob_top_vec_.push_back(this->blob_top_2_);
  LayerParameter layer_param;
...
TYPED_TEST(CuDNNSimpleConvolutionLayerTest, TestSimpleConvolutionGroupCuDNN) {
  Caffe::set_mode(Caffe::GPU);
  LayerParameter layer_param;
  ConvolutionParameter* convolution_param =
      layer_param.mutable_convolution_param();
...
TYPED_TEST(CuDNNSimpleConvolutionLayerTest, TestSobelConvolutionCuDNN) {
  // Test separable convolution by computing the Sobel operator
  // as a single filter then comparing the result
  // as the convolution of two rectangular filters.
  Caffe::set_mode(Caffe::GPU);
...
TYPED_TEST(CuDNNSimpleConvolutionLayerTest, TestGradientCuDNN) {
  Caffe::set_mode(Caffe::GPU);
  LayerParameter layer_param;
  ConvolutionParameter* convolution_param =
      layer_param.mutable_convolution_param();
  ...
  convolution_param->mutable_weight_filler()->set_type("gaussian");
  convolution_param->mutable_bias_filler()->set_type("gaussian");
  CuDNNSimpleConvolutionLayer<TypeParam> layer(layer_param);
  GradientChecker<TypeParam> checker(1e-2, 1e-3);
  checker.CheckGradientExhaustive(&layer, this->blob_bottom_vec_,
      this->blob_top_vec_);
}

TYPED_TEST(CuDNNSimpleConvolutionLayerTest, TestGradientGroupCuDNN) {
  Caffe::set_mode(Caffe::GPU);
  LayerParameter layer_param;
  ConvolutionParameter* convolution_param =
      layer_param.mutable_convolution_param();
